...
@property (strong, nonatomic) NSMutableArray *pendingPushArrDic;

@property (strong, nonatomic) void (^messageTapActionBlock)(NSDictionary *dic);
@end
...
+ (void)showWithNotificationMessage:(NSString *)message andDictionary:(NSDictionary *)dictionary {
    [AGPushNoteView showWithNotificationMessage:message andDictionary:dictionary completion:^{
        //Nothing.
    }];
}
...
+ (void)showWithNotificationMessage:(NSString *)message andDictionary:(NSDictionary *)dictionary completion:(void (^)(void))completion {
    ...
    PUSH_VIEW.infoDictionary = dictionary;
    if (message) {
        [PUSH_VIEW.pendingPushArr addObject:message];
        [PUSH_VIEW.pendingPushArrDic addObject:dictionary];
...
- (void)handlePendingPushJumpWitCompletion:(void (^)(void))completion {
    ...
    id lastObjdic = [self.pendingPushArrDic lastObject];
    if (lastObj) {
        [self.pendingPushArr removeObject:lastObj]; //Remove me from arr
        [self.pendingPushArrDic removeObject:lastObjdic];
        NSString *messagePendingPush = [self.pendingPushArr lastObject]; //Maybe get pending push
        NSDictionary *messagePendingPushDic = [self.pendingPushArrDic lastObject];
        
        if (messagePendingPush) { //If got something - remove from arr, - than show it.
            [self.pendingPushArr removeObject:messagePendingPush];
            [AGPushNoteView showWithNotificationMessage:messagePendingPush andDictionary:messagePendingPushDic completion:completion];
        } else {
...
- (void)messageTapAction {
    if (self.messageTapActionBlock) {
        self.messageTapActionBlock(self.infoDictionary);
